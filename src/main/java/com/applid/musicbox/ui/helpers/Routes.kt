...
sealed class Routes(val route: String) {
    ...
    object Artist : Routes({ b, p -> RoutesBuilder.buildArtistRoute("{${RoutesParameters.ArtistRouteArtistName}}") })
    object Album : Routes({ b, p -> RoutesBuilder.buildAlbumRoute("{${RoutesParameters.AlbumRouteAlbumId}}") })
    object AlbumArtist :
        Routes({ b, p -> RoutesBuilder.buildAlbumArtistRoute("{${RoutesParameters.AlbumArtistRouteArtistName}}") })

    object Genre : Routes({ b, p -> RoutesBuilder.buildGenreRoute("{${RoutesParameters.GenreRouteGenre}}") })
    object Playlist : Routes({ b, p -> RoutesBuilder.buildPlaylistRoute("{${RoutesParameters.PlaylistRoutePlaylistId}}") })
}
